...
public class CurvedText : Text {
	...
	/*protected override void OnValidate() { // Comment out due to being redundant with the ine in update
		base.OnValidate();
		if(radius <= 0.0f) {
			radius = 0.001f;
		}
	...
	protected override void OnPopulateMesh(VertexHelper vh) {	
		...
		for (int i = 0; i < stream.Count; i++) {
			...
			Vector3 offset = Quaternion.Euler(0.0f,0.0f,-percentCircumference*dir*360.0f)*Vector3.up*dir;
			v.position = offset*radius*scaleFactor + offset*v.position.y*dir;
			v.position += Vector3.down*dir*radius*scaleFactor;
